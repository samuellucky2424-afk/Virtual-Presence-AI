...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 h-screen bg-black border-r border-[#18181b] transition-transform duration-300 ease-in-out z-50 flex flex-col w-56 shadow-2xl ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="h-20 flex items-center justify-between px-5 border-b border-[#18181b]">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center overflow-hidden shadow-lg shadow-blue-500/20">
              <BrandIcon className="w-10 h-10" />
            </div>
            <div>
              <span className="text-lg font-bold text-white tracking-tight">Surevideotool</span>
